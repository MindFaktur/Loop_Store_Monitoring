...
def calculate_hrs(query: str, active: str, inactive: str):
    """
    ...
    try:
        ...
        for item in result.fetchall():
            ...
            if not store_result.get(store_id, ""):
            ...
            if store_result.get(store_id, {}):
                if store_result.get(store_id, {}).get(active):
                    store_result[store_id][active] += active_hrs
                    store_result[store_id][inactive] += inactive_hrs
                else:
                    store_result[store_id][active] = active_hrs
                    store_result[store_id][inactive] = inactive_hrs
            else:
                store_result[store_id] = {active: active_hrs, inactive: inactive_hrs}
...
def gen_report():
    ...
    calculate_hrs(last_week_query, "uptime_last_week", "downtime_last_week")
    calculate_hrs(last_day_query, "uptime_last_day", "downtime_last_day")
    calculate_hrs(last_hr_query, "uptime_last_hour", "downtime_last_hour")
    ...
    for status_key, status_val in store_result.items():
        output_file.write(
